...
"""
Excel → Jekyll Collections generator (Memory Palace Edition)
------------------------------------------------------------

This version includes:

• is_homepage column — set to TRUE for the homepage row
• Homepage retains natural collection position but permalink becomes "/"
• Navigation system:
    ←  prev_in_collection
    →  next_in_collection
    ↑  last item of previous collection
    ↓  first item of next collection
• No wrapping at world edges (top or bottom)
• Poetry line breaks preserved
• Compatible with existing CSS/JS (edge-left, edge-right, etc.)
• No broken '#' links
• Managed-by-spreadsheet deletion model
"""
...
# Final NAVIGATION ORDER (no wrap)
COLLECTION_ORDER = [
    "_written-photography",
    "_photograph",
    "_taglines",
...
def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["collection", "slug", "title", "body_md", "excerpt", "order",
                ...
                "keywords", "album", "taglines", "is_homepage"]:
...
def build_navigation_map(df: pd.DataFrame) -> dict:
    """
    Returns a dict keyed by (collection_no_us, slug) with:
      prev_in_collection
      next_in_collection
      prev_collection (last item of previous)
      next_collection (first item of next)
    """
    ...
    # Determine "first" and "last" items in each collection
    ...
    # Build initial prev/next within each collection
    nav_map = {}
    ...
    # Build UP/DOWN across collections
    ...
    for c in existing:
        idx = index[c]
        prev_c = existing[idx-1] if idx-1 >= 0 else None  # no wrap
...
def _norm_coll_for_key(row):
    ...
    return (folder or "").lstrip("_")  # no underscore, to align with nav keys

# Build a stable de-dup key: (collection, title*, body*)
...
# Ensure exactly one homepage row (if multiple marked TRUE, keep the first)
...
``
    ...
    for i, row in df.iterrows():
        ...
        # Preserve all poetry line breaks
...
# Collect the final, canonical row for export
export_rows.append({
    "collection": coll_no_us,
    "is_homepage": is_home,
    "title": title.strip(),
    ...
    "references": references,
    "album": fm.get("album", ""),
    # Store the exact body that went to the file (line breaks preserved)
    "body_md": body,
})
